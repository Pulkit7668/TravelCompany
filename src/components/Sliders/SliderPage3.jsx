...
function SliderPage2() {
  ...
  return (
    <div className='w-5/6 h-[550px] m-auto'>
        <h1 className='text-center md:text-left text-4xl font-bold ml-2 xs:text-2xl'>Top countries to visit in Asia</h1>
      <div className='mt-10'>
        <Slider {...settings}>
          {data.map((d, index) => (
            <div key={index} className='bg-white h-full text-black rounded-xl overflow-hidden relative'>
              ...
              >
                ...
                <div className="absolute flex flex-col p-4 mt-56 mr-10 xs:mr-0 xs:text-center">
                  <h3 className="text-md font-bold text-white xs:text-xl">{d.name}</h3>
                  <p className="text-white text-sm mb-2 xs:text-lg">{d.price}</p>
                </div>
              </div>
